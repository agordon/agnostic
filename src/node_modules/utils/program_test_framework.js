...
function validate_program_tests(tests)
{
	...
	try {
		for(var t in tests) {
			...
			if ("argv" in test) {
				if (!ob_utils.IsArray(test.argv))
					throw "argv is not an array";
				for (var i in test.argv) {
					if (!ob_utils.IsString(test.argv[i]))
						throw "argv[" +i+ "] is not a string" ;
				}
			}

			if ("exit_code" in test) {
				if (!ob_utils.IsInteger(test.exit_code))
					throw "exit_code is not an integer";
			}

			if ("stdin" in test) {
				if (!ob_utils.IsString(test.stdin))
					throw "stdin is not a string";
			}

			if ("stdout" in test) {
				if (!ob_utils.IsString(test.stdout))
					throw "stdout is not a string";
			}

			if ("stderr" in test) {
				if (!ob_utils.IsString(test.stderr))
...
function run_program_tests(prog_name, program_constructor, tests, filesystem)
{
