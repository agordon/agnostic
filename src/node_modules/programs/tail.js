...
ProgramTail.prototype = new ProgramBase();
ProgramTail.prototype.constructor = ProgramTail;

ProgramTail.prototype.parse_command_line=function() {
	var parser = new PosixGetOpt(':1234567890c:(bytes)n:(lines)q(quiet)v(verbose)\u1002(silent)\u1000(help)\u1001(version)f:(follow)F\u1003:(pid)\u1004(retry)s:(sleep-interval)\u1005:(max-unchanged-stats)', this.argv);
...
ProgramTail.prototype.tail_content=function(filename,content) {
	if (this.verbose) {
		if (filename === "-")
...
ProgramTail.prototype.internal_run=function() {
	...
	this.process_files ( this.files, this.tail_content );
