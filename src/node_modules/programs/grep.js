...
//Unsupported GNU Grep options follow, notify the user
	...
	// TODO: catch conversion error, report them as "invalid pattern" to the user.
	...
	var modifiers = (this.ignore_case ? "i" : "");
	if (this.match_words)
		this.patterns = this.patterns.map ( function(x) { return '\\b' + x + '\\b'; });
	if (this.match_entire_line)
		this.patterns = this.patterns.map ( function(x) { return '^' + x + '$'; });
	this.patterns = this.patterns.map( function(x) { return new RegExp(x,modifiers); });
	...
	next_line: for (var i in content) {
		...
		for (var p in this.patterns) {
			...
			if (this.show_line_number) {
				this.runtime.stdout.put_line( (1+parseInt(i,10)).toString() + ":" + line);
				continue next_line;
			}

			// No special options: print the line
