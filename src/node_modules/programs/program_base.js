...
//TODO: what's a "require" in a browser?
...
function ProgramBase() {
	this.program_name = "" ; // to be set by sub-classes
...
ProgramBase.prototype.run = function(process_state,argv) {
	if (! (process_state instanceof ProcessState.ProcessState))
		throw new ProgramExecutionError("internal error: run() called with wrong parameter (1st param is not ProcessState)");

	//Detect uninitialized ProcessState's STDIN/STDOUT/STDERR
	if (process_state.stderr === null)
		throw new ProgramExecutionError("internal error: run() called with process_stae.stderr==null");
	if (process_state.stdout === null)
		throw new ProgramExecutionError("internal error: run() called with process_stae.stdout==null");
	if (process_state.stdin === null)
		throw new ProgramExecutionError("internal error: run() called with process_stae.stdin==null");



	
	if (! IsArray(argv) )
		throw new ProgramExecutionError("internal error: run() called with wrong parameter (2nd param is not array of command-line parameters)");

	if (argv.length===0)
		throw new ProgramExecutionError("internal error: run() called with empty argv");
	argv.forEach(VerifyString);
	...
	try {
		exit_code = ProgramBase.prototype.internal_run.call(this);
	} catch (err) {
