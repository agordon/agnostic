...
/*
Helper function to facilitate common POSIX operation:
  given a list of filenames on the command line,
  load the content of each file and perform an operation on it.
  if a filename is "-" - loads STDIN.
*/
ProgramBase.prototype.process_files = function(files, callback) {
	var content;
	var filename;
	for( var i in files ) {
		filename = files[i];
		if (filename === "-") {
			content = this.runtime.stdin.get_all_lines();
		} else {
			var fl = {} ;
			try {
				filename = path_utils.resolve ( this.runtime.getcwd(), filename ) ;
				fl = this.runtime.getFS().openfile(filename,false);
				content = fl.get_all_lines();
			} catch (e) {
		...
		callback.call(this,filename,content);
	}
}


//TODO:
//Allow other authors and other copyright holders.
ProgramBase.prototype.show_version = function() {
	var version = [
		this.program_name + " (Agnostic Emulation) " + this.runtime.getOS().uname().release,
	...
	this.runtime.stdout.put_lines(version);
}

ProgramBase.prototype.show_help = function() {
	var generic_help = [
		this.program_name + " (Agnostic Emulation) " + this.runtime.getOS().uname().release,
		"Help screen is missing. Please report this bug."
	];
	this.runtime.stdout.put_lines(generic_help);
}

ProgramBase.prototype.error_invalid_argument = function(msg) {
	ob_utils.VerifyString(msg);
...
/* Similar to exit(3) */
ProgramBase.prototype.exit = function(exit_code) {
	if (exit_code === 0)
		throw new ProgramExitSuccess();
	throw new ProgramExitFailure(exit_code);
}

ProgramBase.prototype.error_not_implemented_feature = function(feature,msg) {
	ob_utils.VerifyString(feature);
	ob_utils.VerifyString(msg);
