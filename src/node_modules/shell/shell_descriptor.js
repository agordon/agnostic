...
module.exports = DescribeShellCommand;

function DescribeShellCommand(cmd)
{
	var s = new ShellDescriptor();
	return s.DescribeShellCommand(cmd);
}
...
function Command(obj)
{
	...
	{
	case "SimpleCommand":
		return SimpleCommand(value);
	case "and_or_list":
		return AndOrList(value);
	case "list":
		return List(value);
	case "pipeline":
...
function List(obj)
{
	...
	obj.forEach(function(item){
		var exec_type = ob_utils.GetOneKey(item);
		var exec_char = "" ;
		switch (exec_type)
		{
		case "foreground":
...
function AndOrList(obj)
{
	ob_utils.VerifyArray(obj);
	var cmd = obj[0];
	var list_desc = Command(cmd);
	var i = 1 ;
	while (i<obj.length) {
		var condition = obj[i];
		var cmd = obj[i+1];
		i += 2;
		list_desc = list_desc + " " + condition + " " + Command(cmd);
	}
	return list_desc;
