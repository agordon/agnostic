	...
	*/
	...
	function ProcessPipeline(obj)
	{
		...
		for (var i in pipeline) {
			var cmd = pipeline[i];
			shlog("Pipeline", (parseInt(i)+1) + ". ", "`" + DescribeShellCommand(cmd) + "`");
		}
		...
		for (var i in pipeline) {
			...
			shlog("Pipeline", "running step " + ( parseInt(i)+1) + " of pipeline: ", "`" + DescribeShellCommand(cmd) + "`");
	...
	function ProcessAndOrList(obj)
	{
		...
		shlog("AndOrList","1. ", "`" + DescribeShellCommand(cmd) + "`");
		var i = 1 ;
		while (i<list.length) {
			...
			shlog("AndOrList", (parseInt(i/2)+1) + ". ",
				"`" + DescribeShellCommand(tmp) + "`", "(if previous step " + cond_desc + ")");
		}

		shlog("AndOrList", "running first step in AND/OR list: ","`" + DescribeShellCommand(cmd) + "`");
		exitcode = ProcessCommand(true,cmd);
		...
		while (i<list.length) {
			var condition = list[i];
			var cmd = list[i+1];
			var cmd_desc = DescribeShellCommand(cmd);
			i += 2;
	...
	function ProcessList(obj)
	{
		...
		for (var i in list) {
			...
			shlog("List", (parseInt(i)+1) +". ", "`" + DescribeShellCommand(step_cmd) + "`",
		...
		for (var i in list) {
			...
			var step_type = GetOneKey(step);
			var step_cmd = step[step_type];
			shlog("List", "Running step " + (parseInt(i)+1) +":", "`" + DescribeShellCommand(step_cmd) + "`");
