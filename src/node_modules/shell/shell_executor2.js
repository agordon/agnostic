...
*/
	...
	var exit_code = external_program_callback(new_ps,command_name,tokens);
...
function SimpleCommand(process_state, obj)
{
	...
	if ("command" in obj) {
		...
		if (shell_state.isSpecialBuiltinUtility(command_name)) {
			shell_state.runSpecialBuiltinUtility(process_state,tokens);
		} else if (isBuiltinUtility(command_name)) {
			runBuiltinUtility(tokens);
		} else {
			exit_code = runExternalCommand(process_state,new_assignments,tokens);
		}
	} else {
...
function ArithmeticExpression(process_state, obj)
{
	...
	{
	case 'literal':
		if (ob_utils.IsStrictDecimalIntegerValue(value))
			return parseInt(value,10);
		//else, fall-through and assume it is an environment variable

	case 'envvar':
		return EnvVar(process_state, value);

	case 'arithmetics_op_list':
		return ArithmeticOpList(process_state,value);

	case 'arithmetic':
		return ArithmeticExpression(process_state, value);

	case 'subshell':
		return Subshell(process_state,value);
