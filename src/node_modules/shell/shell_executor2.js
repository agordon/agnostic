...
function Token(process_state,allow_multiple_fields,obj)
{
	...
	obj.forEach(function(item){
		...
		case 'envvar_operation':
			desc = EnvVarOperation(process_state,token_value);
			break;
...
See:
Section "2.6.2 Parameter Expansion" (especially the table summarizing the operations)
...
1. Return the content of the variable, based on the current shell execution environment
*/
function EnvVarOperation(process_state, obj)
{
	...
	if (obj["value"]) {
		//TODO: expand 'word' only if needed?
		word_value = Token(process_state,false,obj["value"]);
	}

	var is_set = shell_state.variable_exists(process_state,varname);
	var current_value = "";
	if (is_set)
		current_value = shell_state.variable_get(process_state,varname);
	var is_set_but_null = (is_set && current_value.length==0);
	var is_set_not_null = (is_set && current_value.length>0);
	...
	case ':=':
		if (is_set_not_null) {
			...
			shell_state.variable_set(process_state,varname,word_value);
			return word_value;
		}
	...
	case ':?':
		if (is_set_not_null) {
			...
			//TODO: return shell error
			throw "Agnostic error: not yet implemented";
		}
	...
	case '%':
	case '%%':
		var anchor_start = (operation=="#" || operation=="##"); /* prefix */
		var anchor_end = (operation=="%" || operation=="%%"); /* suffix */
		var greedy =  (operation=="##" || operation == "%%"); /* longest match */
		word_value = word_value.toString();
		var result = path_utils.__fnmatch_exec(
					word_value, /* shell-glob pattern */
					current_value, /* text */
					anchor_start,
					anchor_end,
					greedy);
...
//		console.log("result = ",JSON.stringify(result,undefined,2));
		...
		if (anchor_start) /* chop prefix or suffix?*/
			return current_value.substr(result.len);
		else
			return current_value.substr(0,current_value.length-result.len);

	default:
		throw "internal error: unknown word expansion operation '" + operation + "'";
	}
}
...
function ArithmeticExpression(process_state, obj)
{
	...
	case 'arithmetics_ternary_op':
		result = ArithmeticTernaryOp(process_state,value);
		break;
...
function ArithmeticTernaryOp(process_state, obj)
{
	ob_utils.VerifyArray(obj);
	var condition = obj[0];
	var true_value = obj[1];
	var false_value = obj[2];
	var value = ArithmeticExpression(process_state, condition);
	value = ArithmeticExpression(process_state,
