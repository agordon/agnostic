...
function StorageObject() {
	...
	this.mode = 438 ; // 0666 octal = 438 decimal
	this.uid  = 1000; // ID of owner
	this.gid  = 1000; // GroupID of owner

	this.get_time = time_utils.get_sys_time; //TODO: don't use global function?
...
StorageObject.prototype.chown = function(new_uid,new_gid) {
	ob_utils.VerifyInteger(new_uid);
	ob_utils.VerifyInteger(new_gid);
	this.uid = new_uid;
	this.gid = new_gid;
...
StorageObject.prototype.chmod = function(new_mode) {
	ob_utils.VerifyInteger(new_mode);
	this.mode = new_mode;
...
StorageObject.prototype.utime = function(new_atime, new_mtime) {
	ob_utils.VerifyInteger(new_atime);
	ob_utils.VerifyInteger(new_mtime);
...
FileObject.prototype.append = function(arr) {
	this.__update_atime();
	this.__update_mtime();
	ob_utils.VerifyArray(arr);

	var count_chars = 0 ;
	for (var i in arr) {
		var line = arr[i];
		ob_utils.VerifyString(line);
