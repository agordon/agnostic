...
//TODO: Is this proper Javascript way of using a module?
//TODO: what about Browser mode (no "require?")
...
/* Terrible hack, due to my misunderstanding of Javascript contexts/domains/namespaces.
 Using:
    var FS = require('os/filesystem');
    if (obj instanceof FS.FilesystemException) {}
 Does not work if it is done in another module.
*/
IsFilesystemException=function(obj)
{
	return (obj instanceof FilesystemException);
}
...
function FileSystem() {
	...
	d.chmod(0755);  //TODO: use umask of calling process
...
FileSystem.prototype.path_exists = function(name) {
	return (FileSystem.prototype.__get_hnode.call(this,name)!==null);
}

FileSystem.prototype.is_directory = function(name) {
	var i = FileSystem.prototype.__get_hnode.call(this,name);
	return (i !== null
	        && ( i["type"] === "directory" ));
}
...
FileSystem.prototype.__validate_hnodes = function() {
	...
	validate_node = function(root,path) {
		if (! ( "type" in root ) )
...
FileSystem.prototype.mkdir = function(name) {
	...
	if (FileSystem.prototype.path_exists.call(this,abspath))
	...
	var inode = FileSystem.prototype.__allocate_inode.call(this);
	...
	var i = FileSystem.prototype.__get_hnode.call(this,dirname);
	i["subnodes"][basename] = { "inode":inode, "type":"directory", "subnodes":{} } ;

	FileSystem.prototype.__validate_hnodes.call(this);
}
...
FileSystem.prototype.openfile = function(name,create_if_needed) {
	...
	var exists = FileSystem.prototype.path_exists.call(this,abspath);
	var is_file = FileSystem.prototype.is_file.call(this,abspath);
	if (exists && !is_file)
	...
	if (!exists) {
		...
		f.chmod(0755);  //TODO: use umask of calling process
		this.inodes[ inode ] = f ;
		...
		var i = FileSystem.prototype.__get_hnode.call(this,abspath);
		var inode = i["inode"];
...
FileSystem.prototype.remove = function(name) {
	...
	//parent directory
	var prnt = FileSystem.prototype.__get_hnode.call(this,dirname);
...
FileSystem.prototype.opendir = function(name) {
	...
	var is_directory = FileSystem.prototype.is_directory.call(this,abspath);
